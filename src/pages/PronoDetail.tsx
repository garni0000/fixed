...
import { ArrowLeft, Trophy, TrendingUp, Shield, Zap, CheckCircle2, XCircle } from 'lucide-react';
...
const PronoDetail = () => {
  ...
  const Icon = typeConfig[prono.type as keyof typeof typeConfig].icon;
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        ...
        <div className="card-premium p-8 max-w-4xl mx-auto">
          <div className="flex items-start justify-between mb-6">
            <div>
              <h1 className="text-4xl font-bold mb-2">{prono.match}</h1>
              <p className="text-muted-foreground flex items-center gap-2">
                <Trophy size={16} />
                {prono.league}
              </p>
            </div>
            <Badge className={`${typeConfig[prono.type as keyof typeof typeConfig].color} flex items-center gap-1`}>
              <Icon size={14} />
              {typeConfig[prono.type as keyof typeof typeConfig].label}
            </Badge>
          </div>

          {prono.status !== 'pending' && (
            <div className={`flex items-center gap-2 mb-6 p-4 rounded-lg ${
              prono.status === 'won' ? 'bg-success/10 text-success' : 'bg-destructive/10 text-destructive'
            }`}>
              {prono.status === 'won' ? (
                <>
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            <div className="bg-secondary/50 p-6 rounded-lg">
              ...
              <p className="text-xl font-bold">{prono.prediction}</p>
            </div>
            
            <div className="bg-secondary/50 p-6 rounded-lg">
              <p className="text-sm text-muted-foreground mb-2">Cote</p>
              <p className="text-xl font-bold text-primary">{prono.odds.toFixed(2)}</p>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-lg p-6">
              ...
              <ul className="space-y-2 text-muted-foreground">
                <li>• Match prévu le {new Date(prono.date).toLocaleDateString('fr-FR')}</li>
